...
```{r plot_part_3c, fig.width=6, fig.height=8}
grid.arrange(
    plot_a, 
    plot_b, 
    plot_c, 
...
Found a good reference to recreating this famous plot [@heiss_minard_ggplot2]
...
-   Map lat/long positions of troop groups from the troop data using `geom_path`

-   Color the lines by direction of troop movement as part of the aesthetics mapping

-   Change the size of the troop lines based on the survivor count

-   Need to add the `lineend = "round"` argument to `geom_path` because without it, the line segments to not appear to be continuous,

-   Need to also adjust the size scaling of the lines with `scale_size`

-   Map the colors specifically to tan and black with `scale_colour_manual`
...
Found more help on merging plots and making borders [@R-ggplot2], [@R-gridExtra], [@R-grid].
...
grid.rect(
    x = unit(0.5, "npc"),  # Center of the page
    y = unit(0.5, "npc"),  # Center of the page
    width = unit(1, "npc"),  # Full width
    height = unit(1, "npc"), # Full height
    gp = gpar(col = "black", 
              fill = NA, 
              lwd = 1)  # Border properties
)
```